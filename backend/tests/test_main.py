...
class TestMainAPI(unittest.TestCase):
    ...
    def test_health_check(self):
        ...
        self.assertEqual(
            response.json(), {"status": "ok", "service": "Genlogs Logistics Search API"}
        )
    ...
    def test_search_routes_missing_payload_fields(self):
        ...
        response_2 = self.client.post(
            "/v1/search", json={"from_city": "", "to_city": "WDC"}
        )
        self.assertEqual(response_2.status_code, 400)
        self.assertEqual(
            response_2.json()["detail"],
            "The 'from_city' and 'to_city' fields are required for the search.",
        )
